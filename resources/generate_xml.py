...
def generate_xml(input_file, random_seed, rooms, areas, pedestrian_names, agents, ensemble, checkpoint,
	...
	with open(input_file, "w") as configuration_file:
		...
		entrance_y_coords = np.zeros(num_spawnroom, dtype=int)
		...
		mean_infection_days_dist = np.zeros(len(disease), dtype=int)
		mean_incubation_days_time = np.zeros(len(disease) * 2, dtype=int)
		mean_incubation_days_dist = np.zeros(len(disease), dtype=int)
		mean_fatality_days_time = np.zeros(len(disease) * 2, dtype=int)
		mean_fatality_days_dist = np.zeros(len(disease), dtype=int)
		mean_end_of_immunization_days_time = np.zeros(len(disease) * 2, dtype=int)
		mean_end_of_immunization_days_dist = np.zeros(len(disease), dtype=int)
		...
		if ensemble == "ON":
			...
			for run in range(nrun):	
				...
				configuration_file.write("\t\t\t\t\"MEAN_FATALITY_DAYS\": [" + ','.join(map(str, mean_fatality_days_dist)) + "],\n")
