...
def generate_xml(input_file, random_seed, rooms, areas, pedestrian_names, agents, ensemble, checkpoint,
	...
	with open(input_file, "w") as configuration_file:
		...
		with open(macro_environment_dir + "ENV_EVENTS_STATTIME.xml", "w") as file:
			file.write("<states><macro_environment><ENV_EVENTS_STATTIME>")
			for k in range(total_number_of_agents_types):
				for j in range(event_length):
					file.write(str(env_events_starttime[k][j]) + ("" if((j == event_length - 1) and (k == total_number_of_agents_types - 1)) else ","))
			file.write("</ENV_EVENTS_STATTIME></macro_environment></states>\n")
		autogenerated_variables_names.write("#define ENV_EVENTS_STATTIME \"ENV_EVENTS_STATTIME\"\n")
