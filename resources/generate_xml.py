...
def generate_xml(input_file, random_seed, rooms, areas, initial_agent_order, pedestrian_names, agents, ensemble,
	...
	with open(input_file, "w") as configuration_file:
		...
		rooms_doors_position = np.zeros((len(vlist), 2), dtype=float)
		...
		autogenerated_variables_names.write("#define ACTUAL_NODE_STAY \"actual_node_stay\"\n")
		autogenerated_variables_names.write("#define ACTUAL_NODE_OBJECT \"actual_node_object\"\n")
		...
		with open(macro_environment_dir + "ROOM_DOORS_POSITION.xml", "w") as file:
			...
			for k in range(len(vlist)):
					for j in range(2):
						file.write(str(rooms_doors_position[k][j]) + ("" if((j == 1) and (k == len(vlist) - 1)) else ","))
