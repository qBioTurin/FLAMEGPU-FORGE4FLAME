				...
				if objects_list != []:
					...
					if door and door != "none" and door != "bottom":
						if door == "left":
							...
							objects[['x', 'y', 'width', 'length']] = objects.apply(
								lambda row: pd.Series({
									'x': width - row['y'] - row['length'],
									'y': row['x'],
									'width': row['length'],
									'length': row['width']
								}), axis=1)
							...
							objects[['x', 'y']] = objects.apply(
								lambda row: pd.Series({
									'x': length - row['x'] - row['width'],
									'y': width - row['y'] - row['length']
								}), axis=1)
						elif door == "right":
...
def generate_xml(input_file, random_seed, rooms, areas, initial_agent_order, pedestrian_names, agents, ensemble,
	...
	with open(input_file, "w") as configuration_file:
		...
		starting_x_range = np.zeros(num_spawnroom * 2, dtype=float)
		starting_z_range = np.zeros(num_spawnroom * 2, dtype=float)
		extern_node = np.zeros(num_spawnroom, dtype=int)
		entrance_y_coords = np.zeros(num_spawnroom + 1, dtype=int)
		...
		for v in vlist:
			...
			if v.type == MapEncoding.SPAWNROOM:
				...
				starting_x_range[spawnroom_id * 2 + 1] = v.coords.southeast[0]
				starting_z_range[spawnroom_id * 2] = v.coords.northwest[1]
				starting_z_range[spawnroom_id * 2 + 1] = v.coords.southeast[1]
				extern_node[spawnroom_id] = v.id
				entrance_y_coords[spawnroom_id] = v.coords.y
				spawnroom_areas_ids[v.area][spawnroom_areas_ids[v.area][0]+1] = v.id
			...
			if v.type != MapEncoding.SPAWNROOM and v.type != MapEncoding.FILLINGROOM and v.type != MapEncoding.STAIR:
				if len(v.objects) > 0:
					...
					rooms_x_objects[v.id] = list(v.objects.loc[:, "x"]) + [0] * (max_objects - len(v.objects) + 1)
					rooms_z_objects[v.id] = list(v.objects.loc[:, "y"]) + [0] * (max_objects - len(v.objects) + 1)
		...
		for i, node in enumerate(extern_node):
			autogenerated_defines.write(str(node) + " ? " + str(i) + ":")
			if i < len(extern_node) - 1:
				autogenerated_defines.write(" (x == ")
		autogenerated_defines.write(" -1")

		for _ in extern_node:
			autogenerated_defines.write(")")
		autogenerated_defines.write(")\n")

		autogenerated_defines.write("#define CHECK_IS_SPAWNROOM(x) ((x == ")
		for i, node in enumerate(extern_node):
			autogenerated_defines.write(str(node) + " ? 1:")
			if i < len(extern_node) - 1:
				autogenerated_defines.write(" (x == ")
		autogenerated_defines.write(" 0")
		...
		compartmental_model_name = disease[0]["disease_model_name"][0]

		autogenerated_defines.write("#define RISK_CLASSES " + str(len(disease)) + "\n\n")
		...
		virus_severity = np.zeros(len(disease) + 1, dtype=float)
		contamination_risk = np.zeros(len(disease) + 1, dtype=float)
		risk_const = np.zeros(len(disease) + 1, dtype=float)
		mean_infection_days_time = np.zeros(len(disease) * 2 + 1, dtype=int)
		mean_infection_days_dist = np.zeros(len(disease) + 1, dtype=int)
		mean_incubation_days_time = np.zeros(len(disease) * 2 + 1, dtype=int)
		mean_incubation_days_dist = np.zeros(len(disease) + 1, dtype=int)
		mean_fatality_days_time = np.zeros(len(disease) * 2 + 1, dtype=int)
		mean_fatality_days_dist = np.zeros(len(disease) + 1, dtype=int)
		mean_end_of_immunization_days_time = np.zeros(len(disease) * 2 + 1, dtype=int)
		mean_end_of_immunization_days_dist = np.zeros(len(disease) + 1, dtype=int)
		...
		for disease_idx, disease_dict in enumerate(disease):
			...
			a, b = parse_distribution(disease_dict["gamma_time"][0], disease_dict["gamma_dist"][0])
			mean_infection_days_time[disease_idx * 2] = int(np.round(1 / a))
			mean_infection_days_time[disease_idx * 2 + 1] = int(np.round(1 / b)) if b > 0 else 0
			mean_infection_days_dist[disease_idx] = distributions[disease_dict["gamma_dist"][0]]

			if "E" in compartmental_model_name:
				a, b = parse_distribution(disease_dict["alpha_time"][0], disease_dict["alpha_dist"][0])
				mean_incubation_days_time[disease_idx * 2] = int(np.round(1 / a))
				mean_incubation_days_time[disease_idx * 2 + 1] = int(np.round(1 / b)) if b > 0 else 0
				mean_incubation_days_dist[disease_idx] = distributions[disease_dict["alpha_dist"][0]]

			if "D" in compartmental_model_name:
				a, b = parse_distribution(disease_dict["lambda_time"][0], disease_dict["lambda_dist"][0])
				mean_fatality_days_time[disease_idx * 2] = int(np.round(1 / a))
				mean_fatality_days_time[disease_idx * 2 + 1] = int(np.round(1 / b)) if b > 0 else 0
				mean_fatality_days_dist[disease_idx] = distributions[disease_dict["lambda_dist"][0]]

			if compartmental_model_name.count("S") == 2:
				a, b = parse_distribution(disease_dict["nu_time"][0], disease_dict["nu_dist"][0])
				mean_end_of_immunization_days_time[disease_idx * 2] = int(np.round(1 / a))
				mean_end_of_immunization_days_time[disease_idx * 2 + 1] = int(np.round(1 / b)) if b > 0 else 0
		...
		for agent_name, agent_info in agents.items():
			...
			flow_length = flow_length + flowid_counts.max()
		...
		env_activity_type = np.full((total_number_of_agents_types, max_number_of_agents_subtype, days_in_a_week, flow_length), 1, dtype=float)
		env_hours_schedule = np.zeros((total_number_of_agents_types, max_number_of_agents_subtype, days_in_a_week, hours_schedule_length), dtype=int)
		env_birth_rates_distr =  np.full((total_number_of_agents_types, days_in_a_week, hours_schedule_length), -1, dtype=int)
		env_birth_rates_distr_firstparam =  np.full((total_number_of_agents_types, days_in_a_week, hours_schedule_length), -1, dtype=int)
		env_birth_rates_distr_secondparam =  np.full((total_number_of_agents_types, days_in_a_week, hours_schedule_length), -1, dtype=int)
		...
		env_sterilisation = np.zeros((days, num_areas, num_rooms_types), dtype=float)
		env_air = np.zeros((days, num_areas, num_rooms_types), dtype=float)
		...
		rooms_type_area = [row[0] for row in rooms_whatif.loc["Ventilation", "Ventilation"]]
		for col in rooms_whatif.columns:
			for id_room, room in enumerate(rooms_type_area):
				if col == "Ventilation":
			...
			for agent_name, agent_info in agents.items():
				...
				for shift_index, shift in enumerate(shifts):
					...
					for weekday, i in days_of_a_week.items():
						...
						if not "Rate" in agent_info["entry_type"][0]:
							...
							for j, eetw in entry_exit_time_weekday.iterrows():
								...
								env_hours_schedule[agent_type_idx][shift_index][i][2 * j] = eetw.loc["EntryTime"]

								if j > 0:
									env_flow_distr[agent_type_idx][shift_index][i][flow_index-1] = distributions["Deterministic"]
									env_flow_distr_firstparam[agent_type_idx][shift_index][i][flow_index-1] = 0
									env_flow_distr_secondparam[agent_type_idx][shift_index][i][flow_index-1] = 0

								for k, f in flow_type.iterrows():
									ft, fa = f.loc["Room"].strip().split("-")

									a, b = parse_distribution(f.loc["Time"], f.loc["Dist"])
									env_flow[agent_type_idx][shift_index][i][flow_index] = MapEncoding.to_value(ft.upper())
									...
									env_flow_agentlinked_timeout[agent_type_idx][shift_index][i][flow_index] = int(f.loc["AgentLinkedTimeout"]) * steps_in_a_minute if f.loc["AgentLinkedTimeout"] != "None" else -1
							...
							for j, eetw in entry_exit_time_weekday.iterrows():
								env_hours_schedule[agent_type_idx][shift_index][i][2 * j] = eetw.loc["EntryTime"]
								env_hours_schedule[agent_type_idx][shift_index][i][2 * j + 1] = eetw.loc["ExitTime"]
						...
						for rf in random_flow:
							...
							env_events_agentlinked_timeout[agent_type_idx][e] = int(rf["AgentLinkedTimeout"]) * steps_in_a_minute if rf["AgentLinkedTimeout"] != "None" else -1
					...
					for i in range(n):
						...
						agent_file.write("\t\t<agent_subtype>" + str(shift_index) + "</agent_subtype>\n")
						agent_file.write("\t</xagent>\n")
		...
		simulation_week = math.ceil(days / days_in_a_week)
		...
		autogenerated_variables_names.write("#define STEER_X \"steerx\"\n")
		autogenerated_variables_names.write("#define STEER_Y \"steery\"\n")
		autogenerated_variables_names.write("#define STEER_Z \"steerz\"\n")
		autogenerated_variables_names.write("#define X_PREV \"x_prev\"\n")
		autogenerated_variables_names.write("#define Y_PREV \"y_prev\"\n")
		autogenerated_variables_names.write("#define Z_PREV \"z_prev\"\n")
		autogenerated_variables_names.write("#define CAN_MOVE \"can_move\"\n")
		autogenerated_variables_names.write("#define SKIP_FLOW \"skip_flow\"\n")
		...
		autogenerated_variables_names.write("#define INFECTED_CONTACT \"infected_contact\"\n")
		...
		autogenerated_variables_names.write("#define AGENT_SUBTYPE \"agent_subtype\"\n")
		...
		for col in agents_whatif.columns:
			for agent_name, _ in agents.items():
				if col == "Mask":
					env_mask_type[:, agent_names[agent_name]["ID"]] = [mask_types[mask] for mask in agents_whatif.loc["Type", col][initial_agent_order[agent_name]][1:]]
					env_mask_fraction[:, agent_names[agent_name]["ID"]] = agents_whatif.loc["Fraction", col][initial_agent_order[agent_name]][1:]
				if col == "Vaccination":
					env_vaccination_fraction[:, agent_names[agent_name]["ID"]] = agents_whatif.loc["Fraction", col][initial_agent_order[agent_name]][1:]
					env_vaccination_efficacy[:, agent_names[agent_name]["ID"]] = agents_whatif.loc["Efficacy", col][initial_agent_order[agent_name]][1:]
					env_vaccination_end_of_immunization_distr[:, agent_names[agent_name]["ID"]] = [distributions[distr.split(", ")[0]] for distr in agents_whatif.loc["Coverage Dist.Days", col][initial_agent_order[agent_name]][1:]]
					env_vaccination_end_of_immunization_distr_firstparam[:, agent_names[agent_name]["ID"]] = [param.split(", ")[1] for param in agents_whatif.loc["Coverage Dist.Days", col][initial_agent_order[agent_name]][1:]]
					env_vaccination_end_of_immunization_distr_secondparam[:, agent_names[agent_name]["ID"]] = [param.split(", ")[2] for param in agents_whatif.loc["Coverage Dist.Days", col][initial_agent_order[agent_name]][1:]]
				if col == "Swab":
					env_swab_sensitivity[:, agent_names[agent_name]["ID"]] = agents_whatif.loc["Sensitivity", col][initial_agent_order[agent_name]][1:]
					env_swab_specificity[:, agent_names[agent_name]["ID"]] = agents_whatif.loc["Specificity", col][initial_agent_order[agent_name]][1:]
					env_swab_distr[:, agent_names[agent_name]["ID"]] = [distributions[distr.split(", ")[0]] for distr in agents_whatif.loc["Dist", col][initial_agent_order[agent_name]][1:]]
		...
		with open(macro_environment_dir + "ROOMS_X_OBJECTS.xml", "w") as file:
			...
			for k in range(len(vlist)):
				for i in range(max_objects+1):
						file.write(str(rooms_x_objects[k][i]) + ("" if((i == max_objects) and (k == len(vlist) - 1)) else ","))
		...
		with open(macro_environment_dir + "ROOMS_Z_OBJECTS.xml", "w") as file:
			...
			for k in range(len(vlist)):
				for i in range(max_objects+1):
						file.write(str(rooms_z_objects[k][i]) + ("" if((i == max_objects ) and (k == len(vlist) - 1)) else ","))
		...
		with open(macro_environment_dir + "ROOMS_LENGTH_OBJECTS.xml", "w") as file:
			...
			for k in range(len(vlist)):
				for i in range(max_objects+1):
						file.write(str(rooms_length_objects[k][i]) + ("" if((i == max_objects) and (k == len(vlist) - 1)) else ","))
		...
		with open(macro_environment_dir + "ROOMS_WIDTH_OBJECTS.xml", "w") as file:
			...
			for k in range(len(vlist)):
				for i in range(max_objects+1):
						file.write(str(rooms_width_objects[k][i]) + ("" if((i == max_objects) and (k == len(vlist) - 1)) else ","))
		...
		with open(macro_environment_dir + "ROOMS_RESOURCES_GLOBAL_OBJECTS.xml", "w") as file:
			...
			for k in range(len(vlist)):
				for i in range(max_objects+1):
						file.write(str(rooms_resources_global_objects[k][i]) + ("" if((i == max_objects) and (k == len(vlist) - 1)) else ","))
		...
		with open(macro_environment_dir + "ROOMS_RESOURCES_GLOBAL_OBJECTS_COUNTER.xml", "w") as file:
			...
			for k in range(len(vlist)):
				for i in range(max_objects+1):
						file.write(str(rooms_resources_global_objects_counter[k][i]) + ("" if((i == max_objects) and (k == len(vlist) - 1)) else ","))
		...
		with open(macro_environment_dir + "ROOMS_RESOURCES_SPECIFIC_OBJECTS.xml", "w") as file:
			...
			for j in range(total_number_of_agents_types):
				for k in range(len(vlist)):
					for i in range(max_objects+1):
							file.write(str(rooms_resources_specific_objects[j][k][i]) + ("" if((i == max_objects) and (k == len(vlist) - 1) and (j == total_number_of_agents_types - 1)) else ","))
		...
		with open(macro_environment_dir + "ROOMS_RESOURCES_SPECIFIC_OBJECTS_COUNTER.xml", "w") as file:
			...
			for j in range(total_number_of_agents_types):
				for k in range(len(vlist)):
					for i in range(max_objects+1):
							file.write(str(rooms_resources_specific_objects_counter[j][k][i]) + ("" if((i == max_objects) and (k == len(vlist) - 1) and (j == total_number_of_agents_types - 1)) else ","))
		...
		with open(macro_environment_dir + "SPAWNROOMS_AREAS_IDS.xml", "w") as file:
			file.write("<states><macro_environment><SPAWNROOMS_AREAS_IDS>")
			for i in range(num_areas):
				for j in range(num_spawnroom + 1):
					file.write(str(spawnroom_areas_ids[i][j]) + ("" if((i == num_areas - 1) and (j == num_spawnroom)) else ","))
			file.write("</SPAWNROOMS_AREAS_IDS></macro_environment></states>\n")
		autogenerated_variables_names.write("#define SPAWNROOMS_AREAS_IDS \"SPAWNROOMS_AREAS_IDS\"\n")
				...
				for x in range(max_number_of_agents_subtype):
					for i in range(days_in_a_week):
						for j in range(flow_length):
							file.write(str(env_flow[k][x][i][j]) + ("" if((i == days_in_a_week - 1) and (j == flow_length - 1) and (x == max_number_of_agents_subtype - 1) and (k == total_number_of_agents_types - 1)) else ","))
		...
		with open(macro_environment_dir + "ENV_FLOW_AREA.xml", "w") as file:
			...
			for k in range(total_number_of_agents_types):
				for x in range(max_number_of_agents_subtype):
					for i in range(days_in_a_week):
						for j in range(flow_length):
							file.write(str(env_flow_area[k][x][i][j]) + ("" if((i == days_in_a_week - 1) and (j == flow_length - 1) and (x == max_number_of_agents_subtype - 1) and (k == total_number_of_agents_types - 1)) else ","))
		...
		with open(macro_environment_dir + "ENV_FLOW_AGENTLINKED.xml", "w") as file:
			...
			for k in range(total_number_of_agents_types):
				for x in range(max_number_of_agents_subtype):
					for i in range(days_in_a_week):
						for j in range(flow_length):
							file.write(str(env_flow_agentlinked[k][x][i][j]) + ("" if((i == days_in_a_week - 1) and (j == flow_length - 1) and (x == max_number_of_agents_subtype - 1) and (k == total_number_of_agents_types - 1)) else ","))
		...
		with open(macro_environment_dir + "ENV_FLOW_AGENTLINKED_TYPE.xml", "w") as file:
			...
			for k in range(total_number_of_agents_types):
				for x in range(max_number_of_agents_subtype):
					for i in range(days_in_a_week):
						for j in range(flow_length):
							file.write(str(env_flow_agentlinked_type[k][x][i][j]) + ("" if((i == days_in_a_week - 1) and (j == flow_length - 1) and (x == max_number_of_agents_subtype - 1) and (k == total_number_of_agents_types - 1)) else ","))
		...
		with open(macro_environment_dir + "ENV_FLOW_DISTR.xml", "w") as file:
			...
			for k in range(total_number_of_agents_types):
				for x in range(max_number_of_agents_subtype):
					for i in range(days_in_a_week):
						for j in range(flow_length):
							file.write(str(env_flow_distr[k][x][i][j]) + ("" if((i == days_in_a_week - 1) and (j == flow_length - 1) and (x == max_number_of_agents_subtype - 1) and (k == total_number_of_agents_types - 1)) else ","))
		...
		with open(macro_environment_dir + "ENV_FLOW_DISTR_FIRSTPARAM.xml", "w") as file:
			...
			for k in range(total_number_of_agents_types):
				for x in range(max_number_of_agents_subtype):
					for i in range(days_in_a_week):
						for j in range(flow_length):
							file.write(str(env_flow_distr_firstparam[k][x][i][j]) + ("" if((i == days_in_a_week - 1) and (j == flow_length - 1) and (x == max_number_of_agents_subtype - 1) and (k == total_number_of_agents_types - 1)) else ","))
		...
		with open(macro_environment_dir + "ENV_FLOW_DISTR_SECONDPARAM.xml", "w") as file:
			...
			for k in range(total_number_of_agents_types):
				for x in range(max_number_of_agents_subtype):
					for i in range(days_in_a_week):
						for j in range(flow_length):
							file.write(str(env_flow_distr_secondparam[k][x][i][j]) + ("" if((i == days_in_a_week - 1) and (j == flow_length - 1) and (x == max_number_of_agents_subtype - 1) and (k == total_number_of_agents_types - 1)) else ","))
		...
		with open(macro_environment_dir + "ENV_ACTIVITY_TYPE.xml", "w") as file:
			...
			for k in range(total_number_of_agents_types):
				for x in range(max_number_of_agents_subtype):
					for i in range(days_in_a_week):
						for j in range(flow_length):
							file.write(str(env_activity_type[k][x][i][j]) + ("" if((i == days_in_a_week - 1) and (j == flow_length - 1) and (x == max_number_of_agents_subtype - 1) and (k == total_number_of_agents_types - 1)) else ","))
				...
				for x in range(max_number_of_agents_subtype):
					for i in range(days_in_a_week):
						for j in range(hours_schedule_length):
							file.write(str(env_hours_schedule[k][x][i][j]) + ("" if((i == days_in_a_week - 1) and (j == hours_schedule_length - 1) and (x == max_number_of_agents_subtype - 1) and (k == total_number_of_agents_types - 1)) else ","))
		...
		with open(macro_environment_dir + "ENV_BIRTH_RATE_DISTR.xml", "w") as file:
			...
			for k in range(total_number_of_agents_types):
					for i in range(days_in_a_week):
						for j in range(hours_schedule_length):
							file.write(str(env_birth_rates_distr[k][i][j]) + ("" if((i == days_in_a_week - 1) and (j == hours_schedule_length - 1) and (k == total_number_of_agents_types - 1)) else ","))
		...
		with open(macro_environment_dir + "ENV_BIRTH_RATE_DISTR_FIRSTPARAM.xml", "w") as file:
			...
			for k in range(total_number_of_agents_types):
				for i in range(days_in_a_week):
					for j in range(hours_schedule_length):
						file.write(str(env_birth_rates_distr_firstparam[k][i][j]) + ("" if((i == days_in_a_week - 1) and (j == hours_schedule_length - 1) and (k == total_number_of_agents_types - 1)) else ","))
		...
		with open(macro_environment_dir + "ENV_BIRTH_RATE_DISTR_SECONDPARAM.xml", "w") as file:
			...
			for k in range(total_number_of_agents_types):
				for i in range(days_in_a_week):
					for j in range(hours_schedule_length):
						file.write(str(env_birth_rates_distr_secondparam[k][i][j]) + ("" if((i == days_in_a_week - 1) and (j == hours_schedule_length - 1) and (k == total_number_of_agents_types - 1)) else ","))
		...
		with open(macro_environment_dir + "ENV_EVENTS_PROBABILITY.xml", "w") as file:
			file.write("<states><macro_environment><ENV_EVENTS_PROBABILITY>")
			for k in range(total_number_of_agents_types):
				for j in range(event_length):
					file.write(str(env_events_probability[k][j]) + ("" if((j == event_length - 1) and (k == total_number_of_agents_types - 1)) else ","))
			file.write("</ENV_EVENTS_PROBABILITY></macro_environment></states>\n")
		autogenerated_variables_names.write("#define ENV_EVENTS_PROBABILITY \"ENV_EVENTS_PROBABILITY\"\n")

		with open(macro_environment_dir + "ENV_EVENTS_STARTTIME.xml", "w") as file:
			file.write("<states><macro_environment><ENV_EVENTS_STARTTIME>")
			for k in range(total_number_of_agents_types):
				for j in range(event_length):
					file.write(str(env_events_starttime[k][j]) + ("" if((j == event_length - 1) and (k == total_number_of_agents_types - 1)) else ","))
			file.write("</ENV_EVENTS_STARTTIME></macro_environment></states>\n")
		autogenerated_variables_names.write("#define ENV_EVENTS_STARTTIME \"ENV_EVENTS_STARTTIME\"\n")

		with open(macro_environment_dir + "ENV_EVENTS_ENDTIME.xml", "w") as file:
			file.write("<states><macro_environment><ENV_EVENTS_ENDTIME>")
			for k in range(total_number_of_agents_types):
				for j in range(event_length):
		...
		with open(macro_environment_dir + "ENV_STERILISATION.xml", "w") as file:
			file.write("<states><macro_environment><ENV_STERILISATION>")
			for i in range(days):
				for k in range(num_areas):
					for j in range(num_rooms_types):
						file.write(str(env_sterilisation[i][k][j]) + ("" if((j == num_rooms_types - 1) and (k == num_areas - 1) and (i == days - 1)) else ","))
			file.write("</ENV_STERILISATION></macro_environment></states>\n")
		autogenerated_variables_names.write("#define ENV_STERILISATION \"ENV_STERILISATION\"\n")

		with open(macro_environment_dir + "ENV_AIR.xml", "w") as file:
			file.write("<states><macro_environment><ENV_AIR>")
			for i in range(days):
				for k in range(num_areas):
					for j in range(num_rooms_types):
						file.write(str(env_air[i][k][j]) + ("" if((j == num_rooms_types - 1) and (k == num_areas - 1) and (i == days - 1)) else ","))
			file.write("</ENV_AIR></macro_environment></states>\n")
		autogenerated_variables_names.write("#define ENV_AIR \"ENV_AIR\"\n")
		...
		virus_parameters = WHOLEmodel["virus_parameters"][0]
		contamination_risk_decreased_with_masks = 0.47
		ngen_base = float(virus_parameters["ngen_base"])
		vl = float(virus_parameters["vl"])
		virus_variant_factor = float(virus_parameters["virus_variant"])
		decay_rate = float(virus_parameters["decay_rate"]) / 3600
		gravitational_settling_rate = float(virus_parameters["gravitational_settling_rate"]) / 3600
		exhalation_efficacy_no_surgical_ffp2_masks = [0, 0.59, 0.9]
		inhalation_efficacy_no_surgical_ffp2_masks = [0, 0.59, 0.9]
		inhalation_rate_pure = float(virus_parameters["inhalation_rate_pure"])
		...
		if ensemble == "ON":
			...
			for run in range(nrun):
				...
				configuration_file.write("\t\t\t\t\"PROPORTIONS\": [" + ','.join(map(str, proportions)) + "],\n")
				configuration_file.write("\t\t\t\t\"MEAN_INCUBATION_DAYS_DISTR\": [" + ','.join(map(str, mean_incubation_days_dist)) + "],\n")
				configuration_file.write("\t\t\t\t\"MEAN_INCUBATION_DAYS\": [" + ','.join(map(str, mean_incubation_days_time)) + "],\n")
				configuration_file.write("\t\t\t\t\"MEAN_INFECTION_DAYS_DISTR\": [" + ','.join(map(str, mean_infection_days_dist)) + "],\n")
				configuration_file.write("\t\t\t\t\"MEAN_INFECTION_DAYS\": [" + ','.join(map(str, mean_infection_days_time)) + "],\n")
				configuration_file.write("\t\t\t\t\"MEAN_END_OF_IMMUNIZATION_DAYS_DISTR\": [" + ','.join(map(str, mean_end_of_immunization_days_dist)) + "],\n")
				configuration_file.write("\t\t\t\t\"MEAN_END_OF_IMMUNIZATION_DAYS\": [" + ','.join(map(str, mean_end_of_immunization_days_time)) + "],\n")
				configuration_file.write("\t\t\t\t\"MEAN_FATALITY_DAYS_DISTR\": [" + ','.join(map(str, mean_fatality_days_dist)) + "],\n")
				configuration_file.write("\t\t\t\t\"MEAN_FATALITY_DAYS\": [" + ','.join(map(str, mean_fatality_days_time)) + "],\n")
				...
				configuration_file.write("\t\t\t\t\"CONTAMINATION_RISK\": [" + ','.join(map(str, contamination_risk)) + "],\n")
			...
			configuration_file.write("\t\t<PROPORTIONS>" + ','.join(map(str, proportions)) + "</PROPORTIONS>\n")
			configuration_file.write("\t\t<MEAN_INCUBATION_DAYS_DISTR>" + ','.join(map(str, mean_incubation_days_dist)) + "</MEAN_INCUBATION_DAYS_DISTR>\n")
			configuration_file.write("\t\t<MEAN_INCUBATION_DAYS>" + ','.join(map(str, mean_incubation_days_time)) + "</MEAN_INCUBATION_DAYS>\n")
			configuration_file.write("\t\t<MEAN_INFECTION_DAYS_DISTR>" + ','.join(map(str, mean_infection_days_dist)) + "</MEAN_INFECTION_DAYS_DISTR>\n")
			configuration_file.write("\t\t<MEAN_INFECTION_DAYS>" + ','.join(map(str, mean_infection_days_time)) + "</MEAN_INFECTION_DAYS>\n")
			configuration_file.write("\t\t<MEAN_END_OF_IMMUNIZATION_DAYS_DISTR>" + ','.join(map(str, mean_end_of_immunization_days_dist)) + "</MEAN_END_OF_IMMUNIZATION_DAYS_DISTR>\n")
			configuration_file.write("\t\t<MEAN_END_OF_IMMUNIZATION_DAYS>" + ','.join(map(str, mean_end_of_immunization_days_time)) + "</MEAN_END_OF_IMMUNIZATION_DAYS>\n")
			configuration_file.write("\t\t<MEAN_FATALITY_DAYS_DISTR>" + ','.join(map(str, mean_fatality_days_dist)) + "</MEAN_FATALITY_DAYS_DISTR>\n")
			configuration_file.write("\t\t<MEAN_FATALITY_DAYS>" + ','.join(map(str, mean_fatality_days_time)) + "</MEAN_FATALITY_DAYS>\n")
			...
			configuration_file.write("\t\t<CONTAMINATION_RISK>" + ','.join(map(str, contamination_risk)) + "</CONTAMINATION_RISK>\n")
		...
		autogenerated_variables_names.write("#define PROPORTIONS \"PROPORTIONS\"\n")
		autogenerated_variables_names.write("#define MEAN_INCUBATION_DAYS_DISTR \"MEAN_INCUBATION_DAYS_DISTR\"\n")
		autogenerated_variables_names.write("#define MEAN_INCUBATION_DAYS \"MEAN_INCUBATION_DAYS\"\n")
		autogenerated_variables_names.write("#define MEAN_INFECTION_DAYS_DISTR \"MEAN_INFECTION_DAYS_DISTR\"\n")
		autogenerated_variables_names.write("#define MEAN_INFECTION_DAYS \"MEAN_INFECTION_DAYS\"\n")
		autogenerated_variables_names.write("#define MEAN_END_OF_IMMUNIZATION_DAYS_DISTR \"MEAN_END_OF_IMMUNIZATION_DAYS_DISTR\"\n")
		autogenerated_variables_names.write("#define MEAN_END_OF_IMMUNIZATION_DAYS \"MEAN_END_OF_IMMUNIZATION_DAYS\"\n")
		autogenerated_variables_names.write("#define MEAN_FATALITY_DAYS_DISTR \"MEAN_FATALITY_DAYS_DISTR\"\n")
		...
		autogenerated_defines.write("#define NUMBER_OF_AGENTS_SUBTYPES " + str(max_number_of_agents_subtype) + "\n")
		for agent, ID in pedestrian_names.items():
			autogenerated_defines.write("#define " + str(agent.upper()) + " " + str(ID) + "\n")

		autogenerated_defines.write("\n")

		autogenerated_defines.write("#define CORRIDOR 0\n")
		autogenerated_defines.write("#define INSIDEROOM 1\n")
		autogenerated_defines.write("#define DOOR 2\n")
		autogenerated_defines.write("#define CPOINT 3\n")
		for me in MapEncoding:
			if me not in [MapEncoding.CORRIDOR, MapEncoding.INSIDEROOM, MapEncoding.DOOR, MapEncoding.CPOINT]:
		...
		autogenerated_defines.write("#define PEDESTRIAN_OFFSET_X_DISTR_IDX 15\n")
		autogenerated_defines.write("#define PEDESTRIAN_OFFSET_Z_DISTR_IDX 16\n\n")
...
def main():
	...
	with open("../src/autogenerated_variables_names.h", "w") as autogenerated_variables_names:
		...
		with open("../src/autogenerated_defines.h", "w") as autogenerated_defines:
			...
			autogenerated_defines.write("#define RADIUS " + str(WHOLEmodel["virus_parameters"][0]["radius"]) + "\n")
