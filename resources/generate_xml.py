...
def generate_xml(input_file, random_seed, rooms, areas, initial_agent_order, pedestrian_names, agents, ensemble, checkpoint,
	...
	epigraph_start_day = 244

	# Load hospitalized values from output5000/Parla/v_stats0[0]-*.out files
	parla_dir = os.path.abspath(os.path.join("../../Epigraph-1.3.FLAMEGPU/EpiGraph-1.3.NN/output5000.1/output5000/Parla"))
	epigraph_hospitalized = [0] * (days + 1)
	for d in range(1, days + 1):
		epigraph_day = epigraph_start_day + d - 1
		filename = os.path.join(parla_dir, f"v_stats0[0]-{epigraph_day}.out")
		val = 0
		if os.path.exists(filename):
			...
			print(f"Warning: Epigraph stats file {filename} not found")
		epigraph_hospitalized[d] = val
	...
	with open(input_file, "w") as configuration_file:
		...
		autogenerated_defines.write("#define EPIGRAPH_START_DAY " + str(epigraph_start_day) + "\n")
		...
		autogenerated_variables_names.write("#define TOTAL_REJECTED \"total_rejected\"\n\n")
		...
		if ensemble == "ON":
			...
			for run in range(nrun):
				...
				configuration_file.write("\t\t\t\t\"EPIGRAPH_HOSPITALIZED\": [" + ','.join(map(str, epigraph_hospitalized)) + "],\n")
			...
			configuration_file.write("\t\t<EPIGRAPH_HOSPITALIZED>" + ','.join(map(str, epigraph_hospitalized)) + "</EPIGRAPH_HOSPITALIZED>\n")
		...
		autogenerated_variables_names.write("#define EPIGRAPH_HOSPITALIZED \"EPIGRAPH_HOSPITALIZED\"\n")
