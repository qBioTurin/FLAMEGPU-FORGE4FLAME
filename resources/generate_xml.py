...
def read_model(room_file, rooms, areas, y_offset, floor, WHOLEmodel, floor_name, types_IDs, max_dimension, max_objects):
	...
	for room in roomsINcanvas:
		...
		if type != "Fillingroom" and type != "Spawnroom" and type != "Stair":
		...
		if door != "none":
			...
			local_graph.add_vertex(center_x, y, center_z, x, z, [int(x), int(z)], [math.ceil(x + dimension_x), math.ceil(z + dimension_z)], MapEncoding.to_code(type.upper()), areas[area]["ID"], yaw, length, width, resources_dataframe, waiting_room_det_dataframe, waiting_room_rand_dataframe, room_matrix, objects, resources_objects_dataframe)
...
def generate_xml(input_file, random_seed, rooms, areas, pedestrian_names, agents, ensemble, env_dims, y_offset, dirname_experiment,
	...
	with open(input_file, "w") as configuration_file:
		...
		rooms_x_objects = np.zeros((len(vlist), max_objects), dtype=int)
		rooms_z_objects = np.zeros((len(vlist), max_objects), dtype=int)
		rooms_length_objects = np.zeros((len(vlist), max_objects), dtype=int)
		rooms_width_objects = np.zeros((len(vlist), max_objects), dtype=int)
		rooms_resources_global_objects = np.zeros((len(vlist), max_objects), dtype=int)
		rooms_resources_global_counter_objects = np.zeros((len(vlist), max_objects), dtype=int)
		rooms_resources_specific_objects = np.zeros((total_number_of_agents_types, len(vlist), max_objects), dtype=int)
		rooms_resources_specific_counter_objects = np.zeros((total_number_of_agents_types, len(vlist), max_objects), dtype=int)
...
def main():
	...
	with open("../src/autogenerated_variables_names.h", "w") as autogenerated_variables_names:
		...
		with open("../src/autogenerated_defines.h", "w") as autogenerated_defines:
			...
			generate_xml(input_file, seed, rooms_info, areas, pedestrian_names, agents, args.ensemble, environment_dimensions, y_offset, args.dirname_experiment + "/",
						WHOLEmodel,	autogenerated_defines, autogenerated_variables_names, days, floors_IDs, types_IDs,
						steps_in_a_day, steps_in_a_hour, steps_in_a_minute, init_week_day, num_counters, step)
