...
class SpatialGraph:
    ...
    def add_vertex(self, x_value: int, y_value: int, z_value: int, x: int, z: int, northwest: list, southeast: list, vtype: MapEncoding, area: int, yaw: float, length: int, width: int, resources: pd.DataFrame, waitingrooms_det: pd.DataFrame, waitingrooms_rand: pd.DataFrame, room_matrix: np.ndarray):
        global first_vertex_id

        self.vertices[vtype].append(Vertex(self.__first_vid, Coordinates(x_value, y_value, z_value, northwest, southeast), x, z, vtype, area, yaw, length, width, resources, waitingrooms_det, waitingrooms_rand, room_matrix))
